...
plan: 02
subsystem: testing
tags: [julia, verification, physics, adjoint, photon-number, taylor-remainder, gnlse]

# Dependency graph
requires:
  - phase: 04-01
    provides: "verification.jl skeleton with VERIF-01 and VERIF-04 already passing"
provides:
...
affects: [phase-05-result-serialization, phase-07-parameter-sweeps]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "try/catch around @testset blocks to ensure report writer executes on test failure"
    - "Taylor remainder test at Nt=2^14 uses epsilon range [1e0, 1e-1, 1e-2, 1e-3] — larger than Nt=2^8 range due to noise floor differences"
    - "Photon number uses abs.(sim['ωs']) — sim['ωs'] is absolute frequency (includes ω₀ already)"
...
requirements-completed: [VERIF-02, VERIF-03]
