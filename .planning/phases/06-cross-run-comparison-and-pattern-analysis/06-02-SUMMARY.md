...
plan: "02"
subsystem: scripts
tags: [cross-run, comparison, run_comparison, jld2, manifest, soliton, phase-decomposition, overlay]
dependency_graph:
  requires:
    - "06-01: compute_soliton_number, decompose_phase_polynomial, plot_cross_run_summary_table, plot_convergence_overlay, plot_spectral_overlay"
    - "05-01: JLD2 result files, manifest.json append-safe schema"
    - "04-02: raman_optimization.jl with PROGRAM_FILE guard, run_optimization function"
  provides:
    - "scripts/run_comparison.jl: Phase 6 entry point for cross-run comparison"
    - "results/images/cross_run_summary_table.png (pending Task 2 execution)"
    - "results/images/convergence_overlay_all_runs.png (pending Task 2 execution)"
    - "results/images/spectral_overlay_SMF28.png (pending Task 2 execution)"
    - "results/images/spectral_overlay_HNLF.png (pending Task 2 execution)"
  affects:
    - "07: parameter sweeps can use run_comparison.jl pattern for loading results"
tech_stack:
  added: []
  patterns:
    - "PROGRAM_FILE guard allows include-safe scripts with a guarded main block"
    - "P_peak = 0.881374 * P_cont / (fwhm_s * rep_rate) for sech^2 pulse peak power conversion"
    - "JSON3.read returns immutable objects; must rebuild as Dict{String,Any} before mutating"
    - "sim[Δt] is in picoseconds; multiply by 1e-12 for seconds before calling decompose_phase_polynomial"
key_files:
  created:
    - path: "scripts/run_comparison.jl"
      changes: "Phase 6 entry point: 5 run configs, manifest loading, soliton N, phase decomposition, 4 figures"
  modified: []
decisions:
  - "Use RC_ prefix for constants (RC_SMF28_GAMMA, RC_HNLF_GAMMA, etc.) to avoid const redefinition conflict if raman_optimization.jl is ever executed before inclusion"
  - "P_peak = 0.881374 * P_cont / (fwhm_s * rep_rate) — 0.881374 is the sech^2 energy factor confirmed in src/simulation/simulate_disp_mmf.jl line 113"
  - "sim_Dt in JLD2 is picoseconds (sim[Δt] = time_window/Nt in ps); must convert to seconds for decompose_phase_polynomial"
  - "Guard all 5 run_optimization calls + figure generation inside PROGRAM_FILE guard so run_comparison.jl itself is include-safe"
metrics:
  duration_minutes: 15
