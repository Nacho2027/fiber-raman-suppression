...
- `build_oracle(...)` in `scripts/phase13_hvp.jl` now takes `log_cost`, `λ_gdd`, and `λ_boundary`, and records the exact HVP surface in metadata.
- `scripts/phase13_hessian_eigspec.jl` now logs and saves the objective surface used for curvature probes.
