...
\begin{tikzpicture}[scale=1.0]
    % Curve (cost function along a 1D slice)
    \draw[thick, gray] plot[smooth, domain=-0.5:4.5] (\x, {0.3*(\x-2)*(\x-2) + 0.5});

    % Points
    \filldraw[blue!70!black] (1.0, 0.8) circle (3pt) node[above left, font=\small] {$\varphi_k$};
    \filldraw[blue!70!black] (3.0, 0.8) circle (3pt) node[above right, font=\small] {$\varphi_{k+1}$};

    % Gradient arrows (tangent lines)
    \draw[thick, red!70!black, -{Stealth[length=2.5mm]}] (1.0, 0.8) -- (0.2, 1.28) node[left, font=\scriptsize] {$\nabla J_k$};
    \draw[thick, red!70!black, -{Stealth[length=2.5mm]}] (3.0, 0.8) -- (3.8, 1.28) node[right, font=\scriptsize] {$\nabla J_{k+1}$};

    % Delta phi and delta g
    \draw[thick, green!50!black, <->] (1.0, 0.3) -- (3.0, 0.3) node[midway, below, font=\small] {$\Delta\varphi = \varphi_{k+1} - \varphi_k$};
    \draw[thick, orange!70!black, <->] (4.3, 0.5) -- (4.3, 1.5) node[midway, right, font=\small, align=left] {$\Delta g = \nabla J_{k+1}$\\$\quad\;\; - \nabla J_k$};

    % Secant condition label
    \node[font=\small, align=center] at (2.0, -0.5) {The gradient changed by $\Delta g$ when we moved by $\Delta\varphi$\\$\Rightarrow$ $H \approx \Delta g / \Delta\varphi$ (secant condition)};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.0]
    % J(alpha) curve
    \draw[thick, black] plot[smooth, domain=0:5] (\x, {3 - 2.5*\x + 0.8*\x*\x - 0.06*\x*\x*\x});
    \node[font=\small] at (5.3, 2.5) {$J(\varphi + \alpha d)$};

    % Axes
    \draw[->] (-0.3, 0) -- (5.5, 0) node[right, font=\small] {step size $\alpha$};
    \draw[->] (0, -0.3) -- (0, 3.5) node[above, font=\small] {$J$};

    % Starting point
    \filldraw[blue!70!black] (0, 3) circle (2pt) node[left, font=\small] {$J(\varphi)$};

    % Sufficient decrease line (Wolfe 1)
    \draw[dashed, red!60!black] (0, 3) -- (5, 0.5) node[right, font=\scriptsize] {Wolfe 1 bound};

    % Too short region
    \fill[red!10] (0, 0) rectangle (0.8, -0.5);
    \node[font=\scriptsize, red!60!black] at (0.4, -0.35) {too short};

    % Too long region
    \fill[red!10] (3.5, 0) rectangle (5, -0.5);
    \node[font=\scriptsize, red!60!black] at (4.25, -0.35) {too long};

    % Goldilocks region
    \fill[green!15] (0.8, 0) rectangle (3.5, -0.5);
    \node[font=\scriptsize, green!50!black] at (2.15, -0.35) {acceptable (Wolfe)};

    % Chosen alpha
    \draw[thick, green!50!black, dashed] (1.8, 0) -- (1.8, 1.05);
    \filldraw[green!50!black] (1.8, 1.05) circle (3pt) node[above right, font=\small] {chosen $\alpha^*$};
\end{tikzpicture}
