...
\usepackage{fancyhdr}
\usepackage{graphicx}
% Gracefully handle missing figure files. If the file is present, render
% at the requested size; if not, show a framed placeholder so the build
...
\newtcolorbox{update}[1]{colback=cyan!5,colframe=cyan!60!black,title=\textbf{Overnight update --- #1}}
...
\begin{update}{2026-04-19/20}
Three parallel sessions re-ran the April integration results on an
honest grid (Phase 21), added a sharpness-aware optimiser sweep with
Hessian spectra (Phase 22), and ran a matched-quadratic baseline at
$L=100$~m (Phase 23). Three surprises, front-loaded here so you don't
need the rest of the doc to know what happened:
\begin{enumerate}[leftmargin=*,topsep=2pt]
  \item \textbf{The $-76.86$~dB SMF-28 single-mode optimum is stable.}
        Phase 22's \texttt{plain} regularisation row on the canonical
        ...
        verification document \S``Hessian is indefinite at L-BFGS
        optima'') and reproduced Session~F's 100~m warm start at
        $-54.77$~dB, not the canonical optimum.
  \item \textbf{We tried to flatten the basin --- and failed.}
        Phase 22 regularised 26 optima across $\{\texttt{plain},
        \texttt{MC}, \texttt{SAM}, \texttt{trH}\}$; \emph{every single
        one remained Hessian-indefinite}. The best shaper-tolerance buy is
        $\texttt{trH}$ regularisation: $+0.058$~rad of $\sigma_{3\text{dB}}$
        for a $10.08$~dB depth cost on the canonical point.
  \item \textbf{The 100~m ``50$\times$ transferability'' story is
        pre-chirp, not structural phase adaptation.}
        Phase 23 ran a pure-quadratic baseline: $\text{GDD}=+4$~ps$^2$
        matches the live warm-start to within $0.46$~dB. What we
        thought was a nonlinear structural fingerprint turns out to be
        generic dispersive pre-chirp.
\end{enumerate}
\textbf{One re-anchoring.} The Phase 13 Hessian-study optima re-anchor
to $-66.61$~dB (SMF-28) and $-86.68$~dB (HNLF) under the honest
grid --- the eigenstructure verdict (indefinite saddles) survives
unchanged. Details in §\ref{sec:april2026-companion}.
\end{update}
...
In decibels: $J_{\text{dB}} = 10\log_{10}(J)$. Our current best: around $-82$~dB on SMF-28 at the short-fiber Pareto point ($L=0.25$~m, $P=0.10$~W, from the 2026-04 parameter sweep) and around $-77$~dB at the $L=0.5$~m canonical config. These are $1\times10^{-8}$ level Raman fractions --- essentially all the pulse energy stays out of the Raman band.
...
\subsection{Long-fiber extension (100~m)}
\label{sec:longfiber}

One of the more surprising results from the 2026-04 campaign is how far
the \emph{shape} of the optimised phase transfers across fibre length.
Take the $\varphi_\text{opt}$ we found for 2~m of SMF-28 at $P=0.05$~W
(found by a normal L-BFGS run in seconds), resample it onto a 100~m
simulation grid without changing anything else, and then just \emph{evaluate}
$J$ at 100~m without running the optimiser: you get $J = -51.5$~dB. No new
optimisation --- the same phase pattern that suppressed Raman in 2~m also
suppresses it in 100~m, which is 50$\times$ further.

If you then let L-BFGS re-optimise from that warm start at 100~m, it digs
out another $\sim$3~dB to reach $J = -54.8$~dB after 25 iterations
(that run hit the wall clock rather than converging, so the ``-54.8~dB'' is
a lower bound on what the configuration can actually reach). And if you
look at the shape of $\varphi_\text{opt}(\omega)$ over the pulse bandwidth,
it is \emph{not} a low-order polynomial --- a weighted quadratic fit
captures under 4\% of the variance, meaning the phase shape that does the
suppression is structurally non-polynomial, not just a chirp.

\begin{intuition}
The usual undergrad story for pre-compensating GVD is ``apply a quadratic
phase $\varphi_\text{GVD}(\omega) = -\tfrac{1}{2}\beta_2 L \omega^2$ and
the pulse stays together.'' Here the optimised phase is NOT close to that
quadratic --- its $a_2$ coefficient is 2--4 orders of magnitude smaller
than $-\beta_2 L/2$ at both 2~m and 100~m. Raman suppression is a
different regime from dispersion compensation, and the optimiser has
found something richer than a chirp.
\end{intuition}

\begin{warning}
Don't over-interpret the 100~m number yet. The simulation window (160~ps)
is smaller than the Stokes-band walk-off at 100~m
($|\beta_2|\cdot L \cdot \Delta\omega_R \approx 177$~ps), so the
\emph{un-shaped} reference bleeds some Stokes energy into the window's
super-Gaussian attenuator. The shaped runs are fine (they don't generate
much Stokes), but the $\Delta J$ between shaped and un-shaped is an
under-estimate of the real advantage. A 320~ps re-run is the cheap fix;
...
\paragraph{We tried to widen the well. The well did not cooperate.}
If the basin is narrow, the obvious thing to try is a regulariser
that rewards a flatter landscape around the optimum. Phase 22 ran
four such recipes --- a Monte-Carlo shaker
(\texttt{MC}), a Sharpness-Aware Minimisation descent direction
(\texttt{SAM}), and a Hessian-trace penalty (\texttt{trH}) ---
against the plain log-dB objective on two operating points, and
resolved the local Hessian spectrum at every recovered optimum.
The robustness Pareto does move: the \texttt{trH} regulariser widens
$\sigma_{3\text{dB}}$ from $0.025$~rad to $0.083$~rad
($+0.058$~rad) on the canonical point, but you pay $10$~dB of depth
for the privilege. Cheaper trades exist --- \texttt{MC} at
$\lambda{=}7.5\times10^{-2}$ gets $+0.014$~rad of tolerance for
$3.85$~dB of depth --- but here is the surprise:
On Pareto-57 the cheapest trade is not \texttt{trH} ---
Monte-Carlo input perturbation at $\lambda = 7.5\times10^{-2}$ gives
$J = -67.98$~dB at $\sigma_{3\text{dB}} = 0.077$~rad ($+0.066$~rad
over the plain baseline), a cheaper tolerance-per-dB trade than any
\texttt{trH} row at this operating point.

\begin{intuition}
Every single one of the 26 regularised optima was still
\emph{Hessian-indefinite}. None of the four flavours converted the
saddle into a positive-definite minimum. Whatever is going on with
the landscape here, flattening it locally is not enough. The
indefinite-saddle geometry looks like a property of the Raman-cost
constraint structure, not an artefact of the plain objective.
\end{intuition}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{phase22_pareto.png}
\caption{Phase 22 depth--tolerance Pareto. Colour is regularisation
flavour, size is strength. SAM (green cluster) gives no tolerance
gain relative to the plain baseline; \texttt{trH} (red) has the
largest dynamic range; \texttt{MC} (blue) offers cheaper lower-$\sigma$
trades. Every marker is still Hessian-indefinite
(\texttt{results/raman/phase22/SUMMARY.md}).}
\label{fig:phase22_pareto}
\end{figure}
...
\begin{warning}
This ``good warm start within a few L-BFGS iterations''
result was tested on 11 \emph{nearby} $(L, P)$ configurations. It
does \emph{not} extend to the $L = 100$~m extrapolation discussed
at the end of this section (§\ref{sec:companion-100m-pre-chirp}):
that suppression turned out to be generic dispersive pre-chirp,
not a transfer of the non-quadratic structure.
\end{warning}
...
\subsection{The 100~m transfer is pre-chirp, not structural}
\label{sec:companion-100m-pre-chirp}

Earlier drafts treated the 100~m warm-start result as evidence that
the non-quadratic spectral-phase structure found at $L{=}2$~m carries
across $50\times$ the fiber length. Phase 23 put that claim to a
matched-quadratic test. On the current reproducible main-checkout
state, the evaluation-only warm-start at $L{=}100$~m gives
$J = -45.52$~dB (the historical Phase 16 number was $-51.50$~dB;
the $\sim 6$~dB gap is unexplained but plausibly another windowing
artefact on the old stored run). Against that live number, a pure
GDD quadratic phase --- one number, nothing else --- reproduces the
suppression almost exactly: $\text{GDD} = +4$~ps$^2 \to J = -45.06$~dB
(only $0.46$~dB shy), and a more trajectory-similar
$\text{GDD} = +1$~ps$^2 \to J = -44.35$~dB. Every trusted run is
clean on energy drift and on the boundary-condition edge check.

Here is the mechanism in plain language. The canonical 185-fs sech$^2$
pulse at $P = 0.05$~W peak has $T_0 \approx 105$~fs and $P_\text{peak}
\approx 2959$~W. Plugging into the soliton number formula gives
$N_\text{sol} = \sqrt{\gamma P_\text{peak} T_0^2 / |\beta_2|} \approx
1.4$: the pulse is just barely a fundamental soliton. The dispersion
length $L_D \approx 0.5$~m and the nonlinear length $L_\text{NL}
\approx 0.26$~m are both sub-meter --- at the canonical 0.5~m fiber
the pulse does its nonlinear dance exactly once.
The Phase 23 matched-quadratic optimum applies a $+4$~ps$^2$ GDD
pre-chirp, which makes the pulse about $360\times$ longer before it
enters the fiber --- roughly 38~ps instead of 185~fs --- and drops
the peak power by the same factor (energy is conserved), from 2959~W
to about 8~W. Over the 100~m fiber the pulse accumulates only
$|\beta_2|L \approx 2$~ps$^2$ of extra dispersion, which partly
compensates the $+4$~ps$^2$ pre-chirp but doesn't unstretch the
pulse. So through the full 100~m the pulse stays stretched to tens
of picoseconds, the instantaneous peak power stays around 10~W, and
the Raman-gain drive is tiny at every $z$. This is the same
mechanism Cornell's Wise group has used to build high-power fiber
oscillators since 2017 (the Mamyshev oscillator architecture):
deliberately stretch the pulse with large dispersion, do the
nonlinear transformation on a weak stretched pulse, then recompress.
The non-quadratic piece of the optimised phase accounts for only
about half a dB of the 45~dB suppression at 100~m; the rest is
straightforward linear dispersion. What we previously called
``non-polynomial structural adaptation across $50\times$ the fiber
length'' was the pulse shaper applying roughly the right amount of
GDD to match a Mamyshev-style stretched-pulse regime; it was not a
transferred nonlinear fingerprint. We retract that earlier framing.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{phase23_warm_vs_gdd_p1_overlay.png}
\caption{The warm-start $\varphi_{\text{opt}}$@$L{=}2$~m propagated
through 100~m of SMF-28 (blue) sits essentially on top of a pure
$+1$~ps$^2$ quadratic chirp (orange) both in spectral evolution and
endpoint suppression (gap $=1.17$~dB).
\texttt{.planning/phases/23-matched-baseline/images/phase23\_warm\_vs\_gdd\_p1\_overlay.png}.}
\label{fig:phase23_overlay}
\end{figure}

\begin{intuition}
``A complicated non-polynomial phase that takes 20 L-BFGS iterations
to find, and transfers across $50\times$ the fiber length'' was a
much better advisor-meeting story than ``you can get essentially the
same suppression with a single dispersive pre-chirp.'' But the
matched-quadratic baseline is cleaner. The honest framing of the
100~m result is: the 2~m warm-start happens to contain roughly the
right amount of GDD for 100~m, and the GDD alone is doing the
suppression. The non-polynomial residual is real --- the quadratic
fit has $R^2 < 0.04$ on the signal band --- but it is not what is
buying the depth at 100~m.
\end{intuition}

\subsection{What did \emph{not} survive}

Claims that appeared in the 48-hour synthesis and subsequent
overnight results did not make it through the audit:
\begin{itemize}
  ...
  \item The 100~m L-BFGS ``optimum'' of $-54.77$~dB is a lower bound
    ...
    not a converged minimum. Phase 21 independently reproduced the
    same number on an honest grid, so the energy-conservation
    story survives; the lower-bound caveat remains. (Audit §S4;
    Phase 21 reanchor \texttt{results/raman/phase21/longfiber100m/}.)
  \item A joint $\{\varphi, A, E\}$ L-BFGS that regressed from
    $-57$~dB to $-23$~dB is a preconditioning bug (mixed units), not
    physics. (Audit §M1.)
  \item The ``$50\times$ length transfer is evidence of nonlinear
    structural adaptation'' framing. Phase 23's matched-quadratic
    baseline shows that a generic GDD already reaches within
    $0.5$--$1.2$~dB of the warm-start suppression at 100~m; the
    extra non-polynomial structure is not doing the work.
    (See §\ref{sec:companion-100m-pre-chirp}.)
  \item The Sweep-1 knee story at $(\text{SMF-28}, L=2$~m$, P=0.2$~W$)$
    as a function of the simple-phase parameterisation size
    $N_\varphi$. Phase 21's honest recovery got every recovered
    point to output edge fraction $\geq 5\%$, even at
    $N_t{=}65{,}536, T{=}216$~ps; the best any point reached was
    $-66.03$~dB, weaker than the original $-68$~dB claim and still
    not pulse-contained. The $L=0.25$~m, $P=0.10$~W reduced
    parameterisation (Pareto-57) cross-validates independently in
    Phase 22 ($-82.56$~dB vs. original $-82.33$~dB) and \emph{does}
    stand, but the $L=2$~m, $P=0.2$~W knee does not.
...
which ones survived. If you see any of these cited anywhere,
