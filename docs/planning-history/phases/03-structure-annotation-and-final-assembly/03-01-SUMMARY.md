...
tags: [metadata, annotation, evolution, comparison, PyPlot]
dependency_graph:
  requires: [02-02-SUMMARY.md]
  provides: [_add_metadata_block!, plot_merged_evolution, metadata kwarg on 3 functions]
  affects: [scripts/visualization.jl, scripts/test_visualization_smoke.jl]
tech_stack:
  added: []
  patterns: [fig.text+transFigure for figure-level annotation, subplots(2,2)+subplots_adjust for merged layout]
key_files:
  modified:
    - scripts/visualization.jl
    - scripts/test_visualization_smoke.jl
decisions:
  - "Use subplots_adjust(right=0.88, top=0.93, bottom=0.06) instead of tight_layout in plot_merged_evolution — tight_layout after add_axes displaces the colorbar (Pitfall 1)"
  - "Metadata block at y=0.01 (bottom-left) using fig.text+transFigure, identical pattern to add_caption!"
  - "Delta-J annotation expanded to full J_before/J_after/Delta-J block (META-02) in both comparison functions"
  - "Updated Test 10 to match new Delta-J display format (Delta-J string instead of ΔJ =)"
metrics:
  duration: 3 minutes
