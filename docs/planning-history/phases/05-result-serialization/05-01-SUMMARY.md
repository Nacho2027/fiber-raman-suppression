...
tags: [jld2, json3, serialization, optimization, julia]

# Dependency graph
requires:
  - phase: 04-correctness-verification
    provides: verified run_optimization pipeline with all 5 configs working correctly
provides:
...
tech-stack:
  added:
    - JLD2 v0.6.3 (HDF5-compatible binary serialization for Julia native types)
    - JSON3 v1.14.3 (fast JSON read/write with pretty-printing support)
  patterns:
    - Append-safe manifest pattern: read-existing/update-or-append/write-back
    - ASCII-safe JLD2 key names for cross-tool compatibility (uomega0 not uω0)
    - @isdefined guard for script-only constants (RUN_TAG) called from interactive context
    - haskey guard for optional fiber dict fields (betas)

key-files:
  created: []
  modified:
    - Project.toml (JLD2 + JSON3 added to [deps] and [compat])
    - Manifest.toml (resolved JLD2 + JSON3 dependency trees)
    - scripts/raman_optimization.jl (store_trace kwarg, jldsave block, manifest block)
...
patterns-established:
  - "Serialization block placement: after boundary diagnostics, before plotting section in run_optimization"
  - "Manifest entry format: scalar summaries only (no arrays) for fast Phase 6 discovery scan"
  - "JLD2 file includes re-propagation fields (phi_opt, uomega0, band_mask) for Phase 6 without re-running"

requirements-completed: [XRUN-01]
