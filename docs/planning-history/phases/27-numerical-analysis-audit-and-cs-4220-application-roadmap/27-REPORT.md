...
**Source:** Independent audit done after the first Phase 27 pass, with all
...
### What the original Phase 27 got right
...
### What the original Phase 27 missed, underplayed, or misframed
...
### Specific code-verified defects not in Phase 27
...
3. **Chirp sensitivity latent bug** (defect 1). Canonical driver may be
   failing silently.
4. **Planning drift** (from Phase 27, unchanged).
5. **Scaling / conditioning of the full-grid φ vector, specifically the
   mixed-unit `sim` dict** (Phase 27 risk + defect 4 + concrete
...
2. **Extend DCT reduced-basis machinery from amplitude to phase.** Small,
   reuses existing code, directly tests Phase 27's over-parameterization
   hypothesis.
3. **Trust-report bundle with running edge-absorption metric and
   ...
   ties together Phase 27's conditioning seed + the absorbing-boundary
   gap surfaced here.
...
This is a refinement — not a replacement — of Phase 27's
...
phases Phase 27 correctly identifies as future work will be built on a
