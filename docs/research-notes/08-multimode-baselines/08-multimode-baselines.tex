...
GRIN-50 model, one shared spectral phase reduced the simulated Raman-band
fraction by \(23.9\,\mathrm{dB}\) on the final \(N_t=8192\) validation grid
while passing corrected temporal-edge diagnostics. A lower-resolution
regularized candidate reached \(31.7\,\mathrm{dB}\), but the conservative
headline is the high-resolution validation. The most important part of the
story is that an earlier large-gain solution was rejected after raw edge
diagnostics exposed temporal-window contamination. The current result is
therefore meaningful, but not a generic experimental claim about arbitrary
multimode fibers.
\end{abstract}

\section*{Claim Status}
\statusbox{qualified simulation result}{The MMF lane is closed/exploring. It is
usable as qualified simulation evidence with clear claim boundaries: shared
spectral phase shaping suppresses Raman-band generation in one idealized
...
The current evidence says yes in a constrained simulation. The accepted
high-resolution case is GRIN-50 with six scalar modes, length
\(L=2\,\mathrm{m}\), power \(P=0.20\,\mathrm{W}\), \(N_t=8192\), and a
\(96\,\mathrm{ps}\) time window. The optimized shared phase reduces the raw
summed Raman-band metric from \(-17.37\,\mathrm{dB}\) to
\(-41.25\,\mathrm{dB}\).
...
The accepted high-resolution run uses \(N_t=8192\), \(96\,\mathrm{ps}\), six
scalar modes, and the same boundary+GDD penalties as the earlier accepted
\(N_t=4096\) candidate. It stopped through the driver-side time limit, but the
patched optimizer returned the best observed phase, wrote the standard image
set, and passed raw-edge diagnostics. This is why the claim is now stronger
than the earlier grid-limited result, while still remaining a qualified
simulation finding rather than a broad MMF generalization.
...
High-res boundary+GDD & \(-41.25\) & \(23.88\,\mathrm{dB}\) & \(3.59\times10^{-13}\) & accepted \\
\bottomrule
\end{tabular}
\caption{All rows use \(L=2\,\mathrm{m}\), \(P=0.20\,\mathrm{W}\), and
\(96\,\mathrm{ps}\). The first three rows use \(N_t=4096\); the high-resolution
row uses \(N_t=8192\). The unregularized row is scientifically useful because
it explains what had to be fixed, but it is not accepted as a valid result.}
\end{table}
...
\caption{Accepted constrained MMF result: the original \(N_t=4096\) candidate reached \(-17.96\rightarrow -49.69\,\mathrm{dB}\); the final \(N_t=8192\) refinement reached \(-17.37\rightarrow -41.25\,\mathrm{dB}\) with raw edge fractions below \(4\times10^{-13}\).}
\end{figure}
...
modes, or experimental wavefront shapers.
...
\begin{itemize}
    ...
    \item The accepted constrained run still gives a \(23.9\,\mathrm{dB}\)
    reduction at \(N_t=8192\) and passes raw edge diagnostics.
...
\begin{itemize}
    ...
    \item \textbf{Grid-refinement gate closed for this lane.} The \(N_t=8192\),
    \(96\,\mathrm{ps}\) refinement completed with standard images and clean raw
    edge diagnostics.
...
\begin{verbatim}
MMF_VALIDATION_SAVE_DIR=results/raman/mmf_window_validation_gdd_nt8192_final \
