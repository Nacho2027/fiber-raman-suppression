...
\subsection*{Provenance and Validation Status}

The headline positive result is the staged amplitude-on-fixed-phase workflow,
not the older broad joint optimizer. The table values are taken from saved run
summaries and standard-image folders, then checked against the current code
path and gradient smoke tests. A later audit found and fixed a boundary
regularizer amplitude-chain-rule issue, so any older amplitude-enabled result
with a nonzero boundary penalty should be treated as evidence for the strategy,
not as a final publication table, unless it has been regenerated or explicitly
matched by a post-fix run.

That distinction matters for presentation: the durable finding is that bounded
amplitude cleanup can improve a good phase solution. The exact last digit of an
old amplitude run is less important than the repeated pattern across the bound
sweep and local checks.
...
\begin{figure}[H]
\centering
\includegraphics[height=0.50\textheight,keepaspectratio]{figures/no_shaping_control_phase_diagnostic.png}

\vspace{0.5em}
\includegraphics[height=0.32\textheight,keepaspectratio]{figures/no_shaping_control_evolution.png}
\caption{No-shaping control group. The top diagnostic records
\(\phi_k=0\), \(A_k=1\), and unit energy scale; the bottom heat map is the
actual no-shaping propagation reference. This is different from the phase-only
reference, which is already optimized.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[height=0.50\textheight,keepaspectratio]{figures/phase_only_reference_phase_diagnostic.png}

\vspace{0.5em}
\includegraphics[height=0.32\textheight,keepaspectratio]{figures/phase_only_reference_evolution.png}
...
\begin{figure}[H]
\centering
\includegraphics[height=0.50\textheight,keepaspectratio]{figures/amplitude_refinement_phase_diagnostic.png}

\vspace{0.5em}
\includegraphics[height=0.32\textheight,keepaspectratio]{figures/amplitude_refinement_evolution.png}
...
\begin{figure}[H]
\centering
\includegraphics[height=0.50\textheight,keepaspectratio]{figures/energy_refinement_phase_diagnostic.png}

\vspace{0.5em}
\includegraphics[height=0.32\textheight,keepaspectratio]{figures/energy_refinement_evolution.png}
