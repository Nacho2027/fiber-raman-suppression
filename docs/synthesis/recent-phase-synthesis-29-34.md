# Recent Phase Synthesis: 29-34

The recent phases did not replace the main workflow. They narrowed the project.

## Keep

- single-mode phase optimization as the baseline;
- reduced-basis and regularization ideas as useful research context;
- result indexing and artifact checks;
- clear caveats for MMF and long-fiber claims.

## Do not promote

- direct joint multivariable optimization;
- Newton/preconditioning as the production optimizer;
- continuation schedules as the default path.

The practical result is a smaller, clearer lab surface.
