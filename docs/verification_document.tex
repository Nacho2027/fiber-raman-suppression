...
\begin{advisory}
\textbf{New (v4): Long-fiber extension at $L=100$\,m (Phase~16 / Session~F).}
Re-optimising the SMF-28 $L=2$\,m $P=0.05$\,W warm-start phase on a 100~m
fiber (Nt~$=32{,}768$, $T=160$~ps, 25 L-BFGS iterations, run hit wall clock,
$\lVert\nabla J\rVert = 0.48$, converged flag \texttt{false}) reached
$J = -54.77$\,dB. The un-re-optimised warm-start evaluation at 100~m already
gave $J = -51.50$\,dB, meaning the 2~m phase shape \emph{transfers across
50$\times$ the length} before any new optimisation. Energy conservation in
both shaped cases is clean (photon-number drift $\le 2\times10^{-3}$, edge
fraction $\le 10^{-5}$).
...
captured by a quadratic. The structural non-polynomial component is the
physics thread worth following.
...
\begin{flagged}
\textbf{Issue 3: Formulation document mentions smoothness, Tikhonov, GDD, and TOD penalties, but code only implements GDD and boundary penalties (R5).} The formulation document (Section headers reference smoothness and Tikhonov regularization) is outdated relative to the current code. The code implements only:
\begin{itemize}
...
\end{itemize}
This discrepancy should be resolved by updating the formulation document.
\end{flagged}
...
179--198 & L-BFGS interface & $\star$ & Cost in dB, grad in linear \\
...
$\star$ = Issue 1 (non-critical). See Section~6.2.
...
§\ref{sec:april-boundary} (output edge fraction $1.0\%$ and
