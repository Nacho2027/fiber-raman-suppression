# scripts/benchmark_synthesis.jl — Phase 33 cross-run synthesis.
#
...
#   - exit_code in JLD2 is stored as String (see benchmark_run.jl
...
# no ensure_deterministic_*). Do NOT include benchmark_run.jl here —
# that file has Pkg.activate + driver main block.
include(joinpath(@__DIR__, "benchmark_common.jl"))
...
function write_synthesis_md(rows; path::AbstractString)
    open(path, "w") do io
        ...
        println(io, "> to the burst VM (see `scripts/benchmark_common.jl`). The remaining 3 configs × 3 start types = 9 slots ")
