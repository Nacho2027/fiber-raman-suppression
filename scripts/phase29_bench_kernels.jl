...
For production numbers, run on the burst VM via:
  ~/bin/burst-run-heavy P29-kernels 'julia -t auto --project=. scripts/phase29_bench_kernels.jl'
...
`abspath(PROGRAM_FILE) == @__FILE__`, so `phase29_report.jl` may `include()` it
...
for key in sort(collect(keys(results_table)))
    ...
    # phase29_roofline_model.jl after the memo is assembled.
