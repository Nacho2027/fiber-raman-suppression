...
# ═══════════════════════════════════════════════════════════════════════════════
#
#   julia --project=. scripts/research/simple_profile/simple_profile_synthesis.jl
#
...
function write_summary(bl, pert, transfer, simpl, verdict_info, fig_paths)
    ...
    open(out, "w") do io
        ...
        println(io)
        println(io, "1. `scripts/research/simple_profile/simple_profile_driver.jl` — baseline + perturbation + transferability stages")
        println(io, "2. `scripts/research/simple_profile/simple_profile_metrics.jl` — gauge-fixed TV, entropy, stationary-point metrics")
        println(io, "3. `scripts/research/simple_profile/simple_profile_synthesis.jl` — figures + this SUMMARY")
        println(io)
...
function write_handoff(verdict_info, simpl)
    ...
    open(out, "w") do io
        ...
        if v == "FLAT_ROBUST"
            ...
            println(io)
            println(io, "Session E can adapt `scripts/research/simple_profile/simple_profile_metrics.jl` for a dense sweep.")
            println(io, "The primitives live in `scripts/primitives.jl` (READ-ONLY): `gauge_fix`,")
            println(io, "`input_band_mask`, `omega_vector`. Metric helpers live in `scripts/research/simple_profile/simple_profile_metrics.jl`:")
