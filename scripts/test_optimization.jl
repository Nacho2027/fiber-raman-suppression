...
@testset "check_boundary_conditions" begin
    ...
    @testset "uses pre-attenuator field when attenuator is present" begin
        attenuator = ones(Nt, 1)
        attenuator[1:6, 1] .= 1e-4
        attenuator[end-5:end, 1] .= 1e-4
        sim_att = Dict("Nt" => Nt, "attenuator" => attenuator)

        ut_physical = zeros(Nt, 1)
        ut_physical[1:6, 1] .= 1.0
        ut_post = attenuator .* ut_physical

        ok, frac = check_boundary_conditions(ut_post, sim_att; threshold=1e-3)
        @test !ok
        @test frac > 0.9
    end
end

@testset "log-cost regularization and chirp-sensitivity regressions" begin
    uω0, fiber, sim, band_mask, _, _ = make_test_problem(Nt=2^7, L=0.05, P=0.01, tw=5.0)
    φ = 0.02 .* randn(sim["Nt"], sim["M"])

    @testset "log_cost gradient matches finite differences with regularizers" begin
